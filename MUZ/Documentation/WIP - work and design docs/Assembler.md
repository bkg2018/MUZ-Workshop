...
* **Z-80 Processor**: MUZ Assembler assembles all documented Z-80 instructions with official timings
* **code and data sections**: the address space can be split into separate code and data sections, each section having its current address. Multiple named sections can be created for both code or data
* **Two Passes Assembly**: the assembler does a first pass to determine the code size of each assembled line and final adress of every label, then a second pass is done to generate the actual code using the first pass results. It allows instructions to use labels which will be defined later in the source files
* **HEX and binary output**: MUZ-assembler takes a main file and assembles it into HEX or binary files
...
* **Widely compatible syntax**: The assembler is 100% compatible with Steve Cousin's Workshop assembler, and features directives and syntaxes from other assemblers
* **Debugging features**: The assembler issues a lot of warnings for syntax or consistency problems in the source files, but also for address range or code section overwriting, improbable JR or DJNZ jumps, and other issues which could be programming errors. These warnings can be suppressed in the source files with directives
* **GUI oriented functions**: The Assembler C++ class features functionalities which are helpfull for a GUI and/or debugger, such as returning the file, line, section, and possibly label for an address


The syntax of MUZ assembler has been made compatible with Steve Cousins's SC Monitor Workshop. The same assember source files generate the same Hex and binary files. Listing files are very similar with minor differences. Unnamed Code and Data sections works identically.

## Assembler Preprocessing

The preprocessing takes place before actual assembling and code generation. *It acts on the files and lines of source code which will be assembled or not*. Preprocessing directives are useful for configuration, to adapt the assembled code to a specific computer setting. The symbols defined by preprocessor `#DEFINE`directive are replaced by their value before assembling takes place, they can also appear in directives like `#IF` to test specific value and decide if a portion of code will be assembled or not.

As an example, a symbol can be defined to set the ROM starting address, the number of an output port or a string holding the name.
...
Remark: the symbols created by `#define` may have a value or simply be defined. If they don't hold a value, they are considered as defined and can be used as an empty string or a true boolean value in expressions. All symbols are considered as true boolean values in expressions.
...
The preprocessor symbols can use any character except space, tabulation, single quote and double quote. However for compatibiity it's better to restrict to the following characters:

* letters `A` to `Z`, `a` to `z`
* digits `0` to `9`
* the underscore `_`

Notice in particular that accentuated characters, `:` or arithmetic operators like `+`  `-`  `*`  `/` may have side effects and generate assembly bugs.
...
Labels and EQU symbols cannot use instruction names.

MUZ-Assembler recognizes documented and undocumented instructions like `SLL`.

### Registers

Register names are not case sensitive: `BC`and `bc` are accepted.

MUZ-Assembler recognizes documented and undocumented registers like `IXH`.
...
Characters can be defined between single quotes, eg. `'H'`. Although you can use any character between code 0 and code 255, it is common pratice to restrict to ASCII codes between 32 (space) and 127, as other characters are always encoding dependent. If you use other characters, you must be sure that your program will run in an environment with a font that will interpret the characters in the same way you intend in your source assembler files.
...
Strings can also be concatenated using the `+` operator, see the Expressions paragraph below.
...
Expressions uses classical C++ operators and can use any parenthesis levels.
...
|  +      | 2          | 2 | character and number  | number  | adds the number to each character code in the string or character: `'A' + 1` will be evaluated to `66`, and `"BC" + 2` will compute as `"DE"`. 
|  +      | 2          | 2 | string  | string  | string concatenation: `"R" + "4"` is `"R4"`
|  -      | 2          | 1 | number  | number  | unary sign change: `-2`
|  -      | 2          | 2 | number  | number  | substraction: `256 - 0x80` comoputes as `128`
...
|  <      | 4          | 2 | string  | boolean | less than: `"PLANE" < "QUESTION"` is true. Lowercase characters are infferior to uppercase characters.
...
In general parenthesis can be used anywhere in exxpressions, however there is a special case where they must surround a numerical expression: it's when an indirect address is expected.

Indirect addressing is a mode where a given address is not used as a direct value, but rather as the address where the actual data is to be found.
...
In this example, the label `store` is defined at current address and will have 2 bytes of space. Later, the `LD (store), HL` will take the content if the `HL` register and put it in the space pointed at by the `store` address.
...
* `(IX+d)`and `(IY+d)` can generally be used where `(HL)` can be, and will add the content of `IX` or `IY` register to a decimal constant `d`to form the indirect address

As a general rule, register names in the middle of numerical expressions will be considered as unknown symbols and have the value `0`.
...
In comparisons, the `*` character is a joker which ends the comparison. If the symbol characters were identical to the string expression until this joker, the result is true.
...
The interesting point in having named sections is that MUZ-Assembler will list the address spaces used by each sec tion at the end of assembly, and will issue warnings for overwriting cases.
...
* *Global* labels starts with a character other than `@` and are followed by `:`, a directive or an instruction. These labels are unique for the whole assembliung process, so if you redefine a global label in a source file it will take the new value. The Assembler will issue a warning if a global label is deffined more than once.

* *Local* labels starts with a `@`. They are always relative to the last previous global label. Instructions trying to use a local label as destination will never find it after the next global label.

Local labels are usefull to put targets in sub-routines or near code within a portion of code. A label named `@Loop` can be used in every sub-routine starting with a global label, each local `@Loop` will be different from the others. However you should avoid redefining a local label within a sub-routine, as only the last one will be known. If you need to use more than one loop labels, the common practice is to add a numeric suffix, eg. `@Loop1` and `@Loop2`. 
...
Remark: a label preceeding the first `.ORG` in current section will be assigned the value `0`. If no section is defined yet, the unnamed `CODE` section, with the current address `0` will be used for this label.
